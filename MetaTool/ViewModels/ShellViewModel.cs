...
namespace MetaTool.ViewModels
{
    ...
    public class ShellViewModel : FlyoutScreen, IShell
    {
        ...
        /// </summary>
        private MetamodelLoader metamodelLoader = new MetamodelLoader();
        ...
        private void MoveToPageTablesCommandEnded(object sender, EventArgs e)
        {
            ...
            this.entities.Clear();
            EntityDict ents = this.metamodelLoader.GetDict<Ent>();
            EntityDict subtypes = this.metamodelLoader.GetDict<SubtRel>();
            foreach (Ent ent in ents.Values)
            {
                if (ent.DepthCnt == 0)
                {
                    EntityViewModel entity = new EntityViewModel(null, ent);
                    this.entities.Add(entity.EntId,entity);
                    this.rootEntities.Add(entity);
                }
                else
                {
                    SubtRel rel = this.LocateSubtypeWhereSonIs(ent, subtypes);
                    if (rel != null)
                    {
                        EntityViewModel parentEntity = this.entities[rel.SupEntId];
                        EntityViewModel entity = parentEntity.AddSon(ent);
                        this.entities.Add(entity.EntId, entity);
                    }
                    else
                    {
        ...
        /// <summary>
        /// Locates the subtype relationship where son is this entity.
        /// </summary>
        /// <param name="ent">The ent.</param>
        /// <param name="subtypes">The subtypes.</param>
        /// <returns></returns>
        private SubtRel LocateSubtypeWhereSonIs(Ent ent, EntityDict subtypes)
        {
            foreach (SubtRel rel in subtypes.Values)
            {
                if (rel.SubEntId == ent.EntId)
                {
                    return rel;
                }
            }
        ...
        private void Build()
        {
            ...
            StructuredBO structuredBO = this.metamodelLoader.GetStructuredBO();
            this.SetProgress("Selecting structures...", 20);
            foreach (EntityViewModel entity in this.entities.Values)
            {
                if ((!entity.IsSelected.HasValue) || (entity.IsSelected.Value))
                {
                    structuredBO.SelectTable(Convert.ToDecimal(entity.EntId));
                }
            }
            this.SetProgress("Generating template container...", 30);
            // Generate template container
            TemplateContainer container = structuredBO.GetContainer();
            // Add own variables.
            ...
            engine.LoadRulesFromXml(this.templatesPath+"\\"+this.SelectedTemplate+"\\TemplateRules.xml");
