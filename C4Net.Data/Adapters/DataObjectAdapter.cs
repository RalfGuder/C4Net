...
using C4Net.Data.Builders;
using C4Net.Data.DAL;
using C4Net.Data.Definitions;
using C4Net.Data.Transactions;

namespace C4Net.Data.Adapters
{
    /// <summary>
    ...
    public class DataObjectAdapter : DataAdapterBase
    {
        ...
        {
            this.commandBuilder = new CommandBuilder(connectionName, type);
        }
        ...
        /// <param name="connectionName">Name of the connection.</param>
        /// <param name="type">The type.</param>
        public DataObjectAdapter(string connectionName, Type type)
            : base(connectionName)
        {
            this.commandBuilder = new CommandBuilder(connectionName, type);
        }
        ...
        /// </summary>
        /// <param name="type">The type.</param>
        public DataObjectAdapter(Type type)
            : base()
        {
            this.commandBuilder = new CommandBuilder(string.Empty, type);
        }
        ...
        public object[] GetPrimaryKeys(object item)
        {
            ...
            {
                result[i] = accesor.GetValue(item, this.commandBuilder.Definition.PrimaryKeys[i]);
            }
            return result;
        }
        ...
        private void FillParameters(IDbCommand command, object item)
        {
            ...
            foreach (IDbDataParameter parameter in command.Parameters)
            {
                ...
                {
                    parameter.Value = DefaultContainer.Get<IConversionManager>().Convert(accesor.GetPropertyType(columnName), parameter.DbType, value);
                }
            }
        }
            ...
            try
            {
                if (reader.Read())
                {
                    ...
                    foreach (DataColumnMapping columnMapping in mapping.ColumnMappings)
                    {
                        ...
                        ColumnDefinition columnDefinition = this.commandBuilder.Definition.Columns[propertyName];
                        ...
                        {
                            value = DefaultContainer.Get<IConversionManager>().Convert(proxy.GetPropertyType(propertyName), columnDefinition.DataType, value);
        ...
        private void FillList(IList list, IDbCommand command, DataTableMapping mapping, int startRecord, int maxRecords, string transactionID = null)
        {
            ...
            try
            {
                ...
                IConversionManager conversions = DefaultContainer.Get<IConversionManager>();
                ...
                foreach (DataColumnMapping columnMapping in mapping.ColumnMappings)
                {
                    ...
                    ColumnDefinition columnDefinition = this.commandBuilder.Definition.Columns[columnMapping.SourceColumn];
