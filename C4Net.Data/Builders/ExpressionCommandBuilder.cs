...
namespace C4Net.Data.Builders
{
    ...
    public class ExpressionCommandBuilder : ExprReviewer
    {
        ...
        public ExpressionCommandBuilder(Expression expression, TableDefinition definition)
            : base()
        {
        ...
        public ColumnExpression[] GetColumnExpressions(Expression expression)
        {
            ...
            {
                list.Add(new ColumnExpression(this.definition.Columns[(expression as MemberExpression).Member.Name].ColumnName));
            }
            else if ((expression is NewExpression) && (expression.NodeType == ExpressionType.New))
            {
                ...
                foreach (Expression arg in newExpression.Arguments)
                {
                    ...
                    {
                        list.Add(new ColumnExpression(this.definition.Columns[(arg as MemberExpression).Member.Name].ColumnName));
                    }
                }
            }
        ...
        protected override Expression ReviewMember(MemberExpression expression)
        {
            if (expression != null)
            {
                ...
                if (info != null)
                {
                    this.commands.Push(new BaseCommand("[" + this.definition.Columns[info.Name].ColumnName + "]"));
                    this.columns.Push(new ColumnExpression(this.definition.Columns[info.Name].ColumnName));
                }
            }
            return expression;
        }
        ...
        public static BaseCommand GetCommand(Expression expression, TableDefinition definition)
        {
