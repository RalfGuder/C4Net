...
namespace C4Net.Templates.Parser
{
    ...
    public class LanguageParser : BaseParser
    {
            ...
            : base((int)LanguageTokenType.Separator,(int)LanguageTokenType.Root, (int)LanguageParserState.Text)
        {
            ...
            this.Commands.Add("foreach", LanguageCommandType.commandForeach);
            this.Commands.Add("if", LanguageCommandType.commandIf);
            this.Commands.Add("else", LanguageCommandType.commandElse);
            this.Commands.Add("end", LanguageCommandType.commandEnd);
        ...
        /// <summary>
        /// Main entry to this class, static method that receives a source string and a container and resolve the string.
        /// </summary>
            ...
            foreach (ParserToken token in listToken.Parameters)
            {
                if (token.TokenType == (int)LanguageTokenType.Identifier)
                {
                    if (this.Commands.ContainsKey(token.Content))
                    {
                        ...
                        switch (commandType)
                        {
                            case LanguageCommandType.commandForeach:
                            case LanguageCommandType.commandIf:
                            case LanguageCommandType.commandElse:
                                currentToken.AddSon(token);
                                tokenStack.Push(currentToken);
                                currentToken = token;
                                break;
                            case LanguageCommandType.commandEnd:
                                ...
                                if (this.Commands.ContainsKey(currentToken.Content))
                                {
                                    ...
                                    if (commandType == LanguageCommandType.commandElse)
                                    {
                                        currentToken = tokenStack.Pop();
                                    }
                                }
        ...
        /// <param name="container">The container.</param>
        /// <returns></returns>
        public string Evaluate(string str, TemplateContainer container)
        {
            builder.Clear();
